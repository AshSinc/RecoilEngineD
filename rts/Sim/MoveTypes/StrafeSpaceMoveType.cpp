...
CR_REG_METADATA(StrafeSpaceMoveType, (
...
static int SelectLoopBackManeuver(
	...
	if (math::fabs(rightdir.y) > 0.05f)
		return StrafeSpaceMoveType::MANEUVER_FLY_STRAIGHT;

	if (groundDist > TurnRadius(turnRadius, spd.w)) {
		if (math::fabs(frontdir.y) <= 0.2f && gsRNG.NextFloat() > 0.3f)
			return StrafeSpaceMoveType::MANEUVER_IMMELMAN_INV;

	} else {
		if (frontdir.y > -0.2f && gsRNG.NextFloat() > 0.7f)
			return StrafeSpaceMoveType::MANEUVER_IMMELMAN;
...
StrafeSpaceMoveType::StrafeSpaceMoveType(CUnit* owner): ASpaceMoveType(owner)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
bool StrafeSpaceMoveType::HandleCollisions(bool checkCollisions) {
	RECOIL_DETAILED_TRACY_ZONE;
...
void StrafeSpaceMoveType::UpdateManeuver()
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void StrafeSpaceMoveType::UpdateAttack()
{
	RECOIL_DETAILED_TRACY_ZONE;
	// if (!isFighter) {
...
void StrafeSpaceMoveType::UpdateBanking(bool noBanking)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void StrafeSpaceMoveType::SetState(ASpaceMoveType::SpacecraftMovementState newState)
{
	RECOIL_DETAILED_TRACY_ZONE;
	// this state is only used by CHoverAirMoveType, so we should never enter it
	//assert(newState != SPACECRAFT_HOVERING);
	...
	if (spacecraftState == SPACECRAFT_LANDED) {
		//assert(newState == SPACECRAFT_TAKEOFF);
		spacecraftState = SPACECRAFT_TAKEOFF;
	} else {
		spacecraftState = newState;
	}

	owner->UpdatePhysicalStateBit(CSolidObject::PSTATE_BIT_MOVING, (spacecraftState != SPACECRAFT_LANDED));
	owner->useAirLos = (spacecraftState != SPACECRAFT_LANDED);
	...
	switch (spacecraftState) {
		...
		case SPACECRAFT_TAKEOFF:
			// should be in the STATE_FLYING case, but these SPACECRAFT
	...
	if (spacecraftState != SPACECRAFT_LANDING) {
...
float StrafeSpaceMoveType::BrakingDistance(float speed, float rate) const
{
	RECOIL_DETAILED_TRACY_ZONE;
	// Denote:
...
void StrafeSpaceMoveType::SetMaxSpeed(float speed)
{
	RECOIL_DETAILED_TRACY_ZONE;
	maxSpeed = speed;
...
void StrafeSpaceMoveType::StartMoving(float3 gpos, float goalRadius)
{
	RECOIL_DETAILED_TRACY_ZONE;
	StartMoving(gpos, goalRadius, maxSpeed);
}

void StrafeSpaceMoveType::StartMoving(float3 pos, float goalRadius, float speed)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void StrafeSpaceMoveType::StopMoving(bool callScript, bool hardStop, bool)
{
	...
	if (spacecraftState != ASpaceMoveType::SPACECRAFT_FLYING && spacecraftState != ASpaceMoveType::SPACECRAFT_LANDING)
		return;
...
bool StrafeSpaceMoveType::SetMemberValue(unsigned int memberHash, void* memberValue) {
	RECOIL_DETAILED_TRACY_ZONE;
